...
function Episode({ id }) {
  // Call useQuery hook to fetch data about the episode with the given id
  const { data, status } = useQuery({
    ...
    // The queryFn is a function that returns a Promise with the data we want to fetch
  });

  // If the status is not 'success', return null and don't render anything
  if (status !== "success") {
    return null;
  }

  // If the status is 'success', render the episode information inside an article element
  return (
    <article key={id}>
...
function Location({ id }) {
  // Call useQuery hook to fetch data about the location with the given id
  const { data, status } = useQuery({
